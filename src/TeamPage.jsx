    ...
    <div className="min-h-screen bg-[#181818] text-white flex flex-col items-center justify-start overflow-x-hidden">
      ...
      <div className="hero-container w-full h-80 relative flex items-center justify-center mb-12 mt-16">
        ...
        <div className="relative z-20 max-w-6xl px-4 flex flex-col md:flex-row items-start justify-center gap-8">
          <div className="flex-shrink-0">
            <h1 className="text-3xl md:text-5xl font-bold text-blue-500 text-center md:text-left">Meet Our Team</h1>
          </div>
          <div className="flex-1">
            <p className="text-base md:text-lg text-white text-justify">Our team brings together expertise across multiple disciplines — including mechanical design, electronics, control systems, and software engineering. This diverse skill set allows us to tackle every aspect of underwater robotics development in-house, from hull design to vision algorithms. We believe that good robots are built at the intersection of different minds, and we work closely across fields to turn ideas into working systems.</p>
          </div>
        </div>
      </div>
      ...
      <style jsx>{`
        ...
        .scroller {
          content: "";
          width: 15px;
          height: calc(100% + 0px); /* span container height */
          opacity: 0.4;
          background: #888;
          position: absolute;
          top: 5px;
        ...
        .tab--3:checked ~ .indicator { transform: translateY(0%); }   /* first item (AY26/27) */
        .tab--2:checked ~ .indicator { transform: translateY(100%); } /* second item (AY25/26) */
        .tab--1:checked ~ .indicator { transform: translateY(200%); } /* third item (AY24/25) */
...
function TeamSubmenu({ year }) {
  const [activeSubmenu, setActiveSubmenu] = useState(year === 'AY24/25' ? 'Members' : 'leads');
...
const teamDescriptions = {
      'AY26/27': {
      leads:
        "Steering the team’s vision, strategy, and cross-functional coordination to drive AquaCore forward.",
      mechanical:
      ...
      webdev:
        "Developing AquaCore’s digital ecosystem — from web platforms to interactive apps that expand outreach and usability.",
      drone:
      ...
      'AY24/25': {
        Members: "Our small but dedicated team combines experience and passion to lead AquaCore’s 2024 innovations from concept to completion.",
      }

};

    const description = teamDescriptions[year]?.[teamId] || "Meet our team members for the year of AY24/25";
  ...
  return (
    <div className="w-full mx-auto">
      <div className="submenu-container">
        {["AY25/26", "AY26/27"].includes(year) && (
        <div className="submenu-wrap" style={{ '--w-label': `calc(100% / ${teams.length})` }}>
      ...
      {["AY25/26", "AY26/27"].includes(year) && (
        <div className="submenu-content mt-4 md:mt-8 p-3 md:p-4 lg:p-6 bg-[#232323] rounded-lg mb-8">
          <h3 className="text-lg md:text-xl lg:text-2xl font-bold text-blue-400 mb-4">
    ...
    >
      <div className={`team-card ${isFlipped ? 'flipped' : ''}`}>
        ...
        <div className="team-card-front ">
          ...
          <p className="text-blue-400 font-semibold mb-2">{role}</p>
        ...
        <div className="team-card-back">
          ...
          <h4 className="text-lg font-bold text-white mb-2">{name}</h4>
          <h5 className="text-sm text-blue-400 font-semibold mb-2">{schoolYear || role}</h5>
