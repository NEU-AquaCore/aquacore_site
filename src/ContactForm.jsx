  ...
  return (
    <section className="w-full bg-[#151515] py-12 px-4">
      <div className="max-w-4xl mx-auto bg-[#232020] rounded-xl p-6 shadow-lg">
        <h3 className="text-3xl font-extrabold text-blue-500 mb-4">Send us a message (currently unavailable暂未开放)</h3>
        ...
        <form onSubmit={onSubmit} className="space-y-4" noValidate>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input name="name" id="name" className="w-full rounded-md px-3 py-2 bg-[#111] border border-[#333] focus:outline-none focus:ring-2 focus:ring-blue-500" />
              {errors.name && <p className="text-xs text-red-400 mt-1">{errors.name}</p>}
            </div>

            <div>
              <label htmlFor="email" className="block text-sm font-semibold mb-1">Email <span className="text-red-400">*</span></label>
              <input name="email" id="email" type="email" className="w-full rounded-md px-3 py-2 bg-[#111] border border-[#333] focus:outline-none focus:ring-2 focus:ring-blue-500" />
              {errors.email && <p className="text-xs text-red-400 mt-1">{errors.email}</p>}
            </div>

            <div>
              <label htmlFor="phone" className="block text-sm font-semibold mb-1">Phone (optional)</label>
              <input name="phone" id="phone" type="tel" className="w-full rounded-md px-3 py-2 bg-[#111] border border-[#333]" placeholder="+86 139 0123 4567" />
            </div>

            <div>
              <label htmlFor="organisation" className="block text-sm font-semibold mb-1">Organisation / University (optional)</label>
              <input name="organisation" id="organisation" className="w-full rounded-md px-3 py-2 bg-[#111] border border-[#333]" placeholder="e.g. NEU Robotics Club" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
            ...
            <div>
              ...
              <label className="inline-flex items-center text-sm text-gray-300">
                <input name="consent" type="checkbox" defaultChecked required className="mr-2 accent-blue-500" />
                By submitting this form, I agree to be contacted by AquaCore. 
              </label>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              type="submit"
              className={`inline-flex items-center gap-2 px-4 py-2 rounded-md font-semibold ${status === 'sending' ? 'bg-blue-300 text-black' : 'bg-blue-500 text-white'}`}
            ...
            {status === 'error' && <span className="text-sm text-red-400">Error sending — try again or email neu_aquacore@163.com</span>}
          </div>
        </form>
      </div>
    </section>
