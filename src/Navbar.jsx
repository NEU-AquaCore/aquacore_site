...
import aquacoreLogo from './assets/logos/aquacoreLogo.png';
...
const sections = [
  ...
  { id: 'vehicles', label: 'Vehicles', path: '/vehicles',
    ...
    //   { id: 'clutch', label: 'Clutch 2026', path: '/clutch' },
  ...
  return (
    ...
    >
      <div className="flex items-center h-8">
        ...
        <img src={aquacoreLogo} alt="AquaCore Logo" className="h-8 w-auto object-contain" />
        </Link>
      </div>
              ...
              >
                {(s.submenu.filter(sub => sub.path !== s.path)).map(sub => (
                  <Link
                    key={sub.id}
                    to={sub.path}
                    className="block px-4 py-2 text-white hover:bg-blue-500 hover:text-white rounded-lg transition"
                  >
                    {sub.label}
                  </Link>
                ))}
              ...
              <div key={s.id}>
                {s.submenu ? (
                  <div>
                    <div className="flex items-center justify-between">
                      <Link
                        ...
                        className="flex-1 text-left px-4 py-3 text-white hover:bg-blue-500 hover:text-white rounded-lg transition"
                      >
                        {s.label}
                      </Link>
                      <button
                        onClick={() => toggleMobileSubmenu(s.id)}
                        className="px-2 py-3 text-white hover:bg-blue-500 hover:text-white rounded-lg transition"
                  ...
                  <Link
                    to={s.path}
                    onClick={() => handleSelect(s.path)}
                    className="block w-full text-left px-4 py-3 text-white hover:bg-blue-500 hover:text-white rounded-lg transition"
                  >
                    {s.label}
                  </Link>
                )}
      ...
      >
        <button
          className={`absolute left-0 top-1/2 -translate-y-1/2 flex items-center justify-center w-10 h-10 rounded-full bg-[#181818] text-white border border-gray-600 transition-all duration-300 ${expanded ? 'bg-blue-500 text-white' : 'hover:bg-blue-500 hover:text-white'}`}
          style={{ zIndex: 2 }}
          tabIndex={0}
        ...
        <input
          ...
          className={`pl-12 pr-4 py-2 rounded-lg bg-[#181818] text-white border border-gray-600 focus:outline-none focus:border-blue-500 w-full transition-all duration-300 ${expanded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
          style={{ zIndex: 10 }}
        />
        {showDropdown && filtered.length > 0 && expanded && (
          <div className="absolute left-0 right-0 top-full pt-2 w-full bg-[#232323] border border-gray-700 rounded-lg shadow-lg z-40">
            {filtered.map(s => (
              s.submenu ? (
                <div key={s.id} className="block w-full text-left px-4 py-2 text-white hover:bg-blue-500 hover:text-white rounded-lg transition">
                  {s.label}
                ...
                <button
                  ...
                  className="block w-full text-left px-4 py-2 text-white hover:bg-blue-500 hover:text-white rounded-lg transition"
                >
                  {s.label}
                </button>
              )
